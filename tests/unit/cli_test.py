...
class TestCheckConfig:
    def test_cli_check_config_valid(self, tmp_path: Path) -> None:
        ...
        yaml.safe_dump({
            ...
            "davmail": {"host": "localhost", "imap_port": 1110, "smtp_port": 1025, "http_proxy_port": 3128},
            "account": {"email": "t@t.com", "display_name": "t", "imap_host": "localhost", "imap_port": 1110, "smtp_host": "localhost", "smtp_port": 1025},
            "llm": {"base_url": "https://api.openai.com/v1", "api_key": "sk-test", "model": "gpt-4o", "temperature": 0.0, "timeout": 30},
            "rules": {"yaml_dir": "rules", "python_rules_file": "rules/rules.py", "nl_rules_dir": "rules", "evolved_dir": "rules/evolved"},
            "pipeline": {"archive_age_months": 6, "confidence_threshold": 0.7, "llm_failure_alert_threshold": 5},
    ...
    def test_cli_check_config_invalid(self, tmp_path: Path) -> None:
        ...
        from kontor_cli.cli import cli
        runner = CliRunner()
        result = runner.invoke(cli, ["check-config", "--config", str(tmp_path / "missing.yaml")])
...
class TestClassify:
    def test_cli_classify_output_format(self, tmp_path: Path) -> None:
        ...
        mock_email = Email(id="42", from_addr="alice@example.com", subject="Test", date=datetime.now(UTC), flags={}, folder="INBOX")

        with mock.patch("kontor_cli.cli.Config.load", return_value=mock_cfg):
            with mock.patch("kontor_cli.himalaya.list_emails", return_value=[mock_email]):
                with mock.patch("kontor_cli.rules_engine.RulesEngine") as mock_re:
                    ...
                    from kontor_cli.cli import cli
                    runner = CliRunner()
                    result = runner.invoke(cli, ["classify", "--email-id", "42"], catch_exceptions=False)
    ...
    def test_cli_process_rebuild(self, tmp_path: Path) -> None:
        ...
        with mock.patch("kontor_cli.cli.Config.load", return_value=mock_cfg):
            with mock.patch("kontor_cli.cli.RebuildPipeline") as mock_rebuild:
                ...
                from kontor_cli.cli import cli
                runner = CliRunner()
                result = runner.invoke(cli, ["process", "--phase", "rebuild", "--config", str(cfg_file)])
                assert result.exit_code == 0
